...
public interface StockClient {
    ...
    @PostMapping("/warmup")
    ApiResponse<StockWarmupResultDTO> warmupStocks(@RequestBody StockWarmupRequestDTO request);

    @PostMapping("/pre-deduct")
    ApiResponse<StockPreDeductResultDTO> preDeductByRedis(@RequestBody StockPreDeductRequestDTO request);

    @PostMapping("/rollback")
    ApiResponse<Void> rollbackRedisStock(@RequestBody StockRollbackRequestDTO request);
}
