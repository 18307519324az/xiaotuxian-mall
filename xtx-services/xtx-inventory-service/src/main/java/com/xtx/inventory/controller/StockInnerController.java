...
public class StockInnerController {
    ...
    @PostMapping("/inner/stocks/warmup")
    public ApiResponse<StockWarmupResultDTO> warmup(@RequestBody StockWarmupRequestDTO request) {
        return ApiResponse.success(inventoryAppService.warmupStocks(request));
    }

    @PostMapping("/inner/stocks/pre-deduct")
    public ApiResponse<StockPreDeductResultDTO> preDeduct(@RequestBody StockPreDeductRequestDTO request) {
        return ApiResponse.success(inventoryAppService.preDeductByRedis(request));
    }

    @PostMapping("/inner/stocks/rollback")
    public ApiResponse<Void> rollback(@RequestBody StockRollbackRequestDTO request) {
        inventoryAppService.rollbackRedisStock(request);
